...
create table if not exists cartisan.promotions (
  ...
  category_id text references cartisan.catalog_categories (id),
...
create index if not exists commerce_orders_promotion_idx on cartisan.commerce_orders (promotion_id, created_at);
...
create index if not exists merchant_changes_conversation_idx on cartisan.merchant_changes (conversation_id, created_at);
